...
const createArticleValidation = [
    ...
    (0, express_validator_1.body)("pourcentage_promo")
        .notEmpty()
    ...
    (0, express_validator_1.body)("imgsID")
        .isArray({ min: 1 })
        .withMessage("Veuillez fourni les ids des images pour cette article !"),
    (0, express_validator_1.body)("idCategorie")
        .notEmpty()
    ...
    (0, express_validator_1.body)("featured").notEmpty().withMessage(" article en vedette pas fourni !").custom((value, { req }) => {
        if (value === true) {
            if (!req.body.collection || !req.body.position) {
                throw new Error("Si l'image est définie en vedette, la position et la collection doivent également être définies !");
            }
        }
        return true;
    }),
...
const updateArticleImg = [
    ...
    (0, express_validator_1.body)("position").notEmpty().withMessage("Position non défini"),
    (0, express_validator_1.body)("featured").notEmpty().withMessage(" article en vedette pas fourni !").custom((value, { req }) => {
        if (value === true) {
            if (!req.body.collection || !req.body.position) {
                throw new Error("Si l'image est définie en vedette, la position et la collection doivent également être définies !");
...
const createCommandeValidation = [
    (0, express_validator_1.body)("article")
        .isArray({ min: 1 })
        .withMessage("Le champ 'article' doit être un tableau contenant au moins un article"),
    ...
    (0, express_validator_1.body)("isExpressOrder").notEmpty()
...
const createInvoiceValidation = [
    (0, express_validator_1.body)("idCommande")
        .notEmpty()
        .withMessage("ID de la commande est obligatoire !"),
    (0, express_validator_1.body)("idArticles")
        .notEmpty()
