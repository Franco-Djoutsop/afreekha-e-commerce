...
const gestion_express_user_1 = require("../repositry/gestion_express_user");
            ...
            try {
                if (!req.body.errors) {
                    ...
                    if (data.article.length != 0) {
                        ...
                        const addedUser = data.isExpressOrder ? yield gestion_express_user_1.GestionExpressUser.create(data.userData) : null;
                        ...
                        if (verificationRslt.exist) {
                            ...
                            if (resp) {
                                data.article.forEach((art) => {
                                    ...
                                    const index = verificationRslt.articles.findIndex((val) => val.idArticle === art.product_id);
            ...
            try {
                if (req.params.idUser) {
                    ...
                    return result.length != 0
                        ? res.status(200).json([{ data: crypto_js_1.crypt.encode(result) }])
                        : res.status(200).json([]);
                }
                else {
